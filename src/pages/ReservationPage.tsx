    ...
    return (
      <main className="reservation-page">
        <nav className="reservation-navbar">
          <button type="button" onClick={() => navigate("/")}>Hotel Rolex</button>
        ...
        <section className="confirmation-card">
          ...
          <div className="confirmation-actions">
            <button type="button" onClick={() => navigate("/habitaciones")}>Volver a habitaciones</button>
            <button type="button" className="outline" onClick={() => navigate("/")}>Ir al inicio</button>
          </div>
        </section>
      </main>
    );
    ...
    <main className="reservation-page">
      ...
      <section className="reservation-layout">
        <div className="reservation-intro">
          <span>HOTEL ROLEX · RESERVAS</span>
          <h1>Haz tu reserva.</h1>
          <p>Completa tus datos, selecciona las fechas y confirma tu habitación en pocos pasos.</p>
          ...
          <div className="reservation-steps"><div><b>01</b><span>Elige habitación</span></div><div><b>02</b><span>Completa tus datos</span></div><div><b>03</b><span>Confirma</span></div></div>
        </div>
        ...
        <form className="reservation-form" onSubmit={handleSubmit}>
          ...
          <div className="form-grid">
            <label>Fecha de entrada<input type="date" value={checkIn} min={today} onChange={(e) => { setCheckIn(e.target.value); if (checkOut && e.target.value >= checkOut) setCheckOut(""); }} required /></label>
            <label>Fecha de salida<input type="date" value={checkOut} min={checkIn ? new Date(new Date(`${checkIn}T00:00:00`).getTime() + 86400000).toISOString().slice(0,10) : today} onChange={(e) => setCheckOut(e.target.value)} required /></label>
          </div>
          ...
          <button className="confirm-button" type="submit">Confirmar reserva <span>→</span></button>
          <p className="saved-note">Las reservas de esta versión se almacenan localmente en tu navegador.</p>
        </form>
      </section>
    </main>
  );
